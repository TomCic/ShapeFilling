 ...
 * @return the zip archive handler or NULL on error
 */
extern ZIP_EXPORT struct zip_t *zip_openwitherror(const char *zipname,
                                                  int level, char mode,
                                                  int *errnum);
 ...
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
extern ZIP_EXPORT int zip_stream_extract(
    const char *stream, size_t size, const char *dir,
